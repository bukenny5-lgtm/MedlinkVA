...
export function InfoCard({ eyebrow, title, description, bullets, footer, className = "" }: InfoCardProps) {
  return (
    <article className={`surface-card flex h-full flex-col p-6 ${className}`}>
      {eyebrow ? <p className="text-xs font-semibold uppercase tracking-[0.24em] text-brand-accent">{eyebrow}</p> : null}
      <h3 className="mt-2 break-words text-xl font-semibold text-brand-navy">{title}</h3>
      {description ? <p className="mt-3 text-sm leading-7 text-brand-charcoal/80">{description}</p> : null}
