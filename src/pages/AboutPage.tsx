...
export function AboutPage() {
  ...
  return (
    <article className="space-y-12">
      ...
      {partners.length ? <HomeSection className="bg-brand-background py-16 sm:py-20"><SectionHeading eyebrow="Partners & collaborations" title="Organisations connected with MedLink VA" description="MedLink VA partners with the organisations listed below. Logos and links can be added through Sanity when approved assets are available." /><div className="mt-8 grid gap-4 sm:grid-cols-3">{partners.map((partner) => <article key={partner.name} className="surface-card flex min-h-24 items-center justify-center p-5 text-center">{partner.websiteUrl ? <a href={partner.websiteUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-brand-navy underline-offset-4 hover:underline">{partner.logo ? <img src={sanityImageSrc(partner.logo, { width: 500 }) ?? ""} alt={partner.altText || `${partner.name} logo`} className="max-h-14 max-w-full object-contain" loading="lazy" decoding="async" /> : partner.name}<span className="sr-only"> (opens in a new tab)</span></a> : partner.logo ? <img src={sanityImageSrc(partner.logo, { width: 500 }) ?? ""} alt={partner.altText || `${partner.name} logo`} className="max-h-14 max-w-full object-contain" loading="lazy" decoding="async" /> : <span className="font-semibold text-brand-navy">{partner.name}</span>}</article>)}</div></HomeSection> : null}
