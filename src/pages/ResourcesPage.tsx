...
function ResourceCard({ post }: { post: ReturnType<typeof resolveResourcesContent>["records"][number] }) {
  ...
  return <article className="surface-card group flex h-full flex-col overflow-hidden transition hover:-translate-y-1 hover:shadow-lg">{image ? <img src={image} alt={post.coverImageAlt || ""} className="aspect-video w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" loading="lazy" decoding="async" /> : null}<div className="flex flex-1 flex-col gap-3 p-6"><div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-brand-accent"><span>{post.resourceType || post.category}</span><span>{readTime(post.excerpt)}</span>{post.publishedAt ? <span>{formatDate(post.publishedAt)}</span> : null}</div><h3 className="text-2xl font-semibold text-brand-navy">{post.title}</h3><p className="text-sm leading-7 text-brand-charcoal/80">{post.excerpt}</p><div className="mt-auto pt-2">{slug ? <Link to={`/resources/${slug}`} className="btn-secondary">{label} <span aria-hidden="true">→</span></Link> : null}</div></div></article>;
}
