...
function ResourceCard({ post }: { post: ReturnType<typeof resolveResourcesContent>["records"][number] }) {
  ...
  const label = post.callToActionLabel || (post.externalUrl || post.downloadFile?.asset?.url ? "View Resource" : "Published Resource");
  return <article className="surface-card flex h-full flex-col overflow-hidden">{image ? <img src={image} alt={post.coverImageAlt || post.title} className="aspect-[3/2] w-full object-cover" loading="lazy" decoding="async" /> : null}<div className="flex flex-1 flex-col gap-3 p-6"><div className="flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-brand-accent"><span>{post.resourceType || post.category}</span>{post.publishedAt ? <span>{formatDate(post.publishedAt)}</span> : null}</div><h3 className="text-2xl font-semibold text-brand-navy">{post.title}</h3><p className="text-sm leading-7 text-brand-charcoal/80">{post.excerpt}</p><div className="mt-auto pt-2">{post.externalUrl ? <a href={post.externalUrl} target="_blank" rel="noopener noreferrer" className="btn-secondary">{label} <span aria-hidden="true">↗</span></a> : post.downloadFile?.asset?.url ? <a href={post.downloadFile.asset.url} target="_blank" rel="noopener noreferrer" className="btn-secondary">{label} <span aria-hidden="true">↗</span></a> : null}</div></div></article>;
}
